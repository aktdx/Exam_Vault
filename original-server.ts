...
import { 
  ...
  examTypes, questionPapers, downloads, users
} from "./src/db/schema.ts";
import { eq, and, ilike, or, sql } from "drizzle-orm";
import { uploadFile, getFileUrl, deleteFile } from "./src/lib/storage.ts";
...
async function startServer() {
  ...
  // Auth Me endpoint
  app.get("/api/v1/auth/me", requireAuth, async (req: any, res) => {
    res.json({
      user: {
        uid: req.dbUser.uid,
  ...
  app.get("/api/v1/admin/users", requireAuth, requireAdmin, async (req: any, res) => {
    if (req.dbUser.email !== 'aaminkhansohel@gmail.com') {
  ...
  app.put("/api/v1/admin/users/:uid/toggle-admin", requireAuth, requireAdmin, async (req: any, res) => {
    if (req.dbUser.email !== 'aaminkhansohel@gmail.com') {
      return res.status(403).json({ error: 'Forbidden: Super Admin access required' });
    }
    try {
      ...
      const targetUser = await db.select().from(users).where(eq(users.uid, uid));
      if (!targetUser.length) {
        return res.status(404).json({ error: 'User not found' });
      }
      if (targetUser[0].email === 'aaminkhansohel@gmail.com') {
        return res.status(400).json({ error: 'Cannot modify super admin' });
      }

      await db.update(users).set({ isAdmin }).where(eq(users.uid, uid));
  ...
  app.post("/api/v1/admin/users/add-admin", requireAuth, requireAdmin, async (req: any, res) => {
    if (req.dbUser.email !== 'aaminkhansohel@gmail.com') {
      return res.status(403).json({ error: 'Forbidden: Super Admin access required' });
    }
    try {
      ...
      try {
        ...
        if (fbError.code === 'auth/user-not-found') {
          return res.status(404).json({ error: 'User has never logged into this app before. Please ask them to log in once first.' });
        }
        throw fbError;
      }
    } catch (err: any) {
  ...
  app.get("/api/v1/admin/question-papers/trash", requireAuth, requireAdmin, async (req, res) => {
    try {
      const papers = await db.select({
        id: questionPapers.id,
        year: questionPapers.year,
  ...
  app.put("/api/v1/admin/question-papers/:id/restore", requireAuth, requireAdmin, async (req, res) => {
    try {
      await db.update(questionPapers).set({ isDeleted: false }).where(eq(questionPapers.id, Number(req.params.id)));
      res.json({ success: true });
    } catch (err: any) { res.status(500).json({ error: err.message }); }
  });

  app.delete("/api/v1/admin/question-papers/:id/hard", requireAuth, requireAdmin, async (req, res) => {
    try {
      const paperId = Number(req.params.id);
      
      const existingPaper = await db.select().from(questionPapers).where(eq(questionPapers.id, paperId));
      if (existingPaper.length > 0) {
          const fileUrl = existingPaper[0].fileUrl;
          if (fileUrl) {
            try {
  ...
  // Catch-all for API routes to prevent serving index.html
  app.all("/api/*", (req, res) => {
